...
where
	...
	<Client<Backend, Exec, Block, RtApi> as ProvideRuntimeApi<Block>>::Api:
		BlockBuilder<Block, Error = sp_blockchain::Error> + ApiExt<Block, StateBackend = Backend::State>,
	...
	Block::Hash: From<sp_core::H256>,
{
	...
	while factory_state.block_number() < factory_state.blocks() {
		...
		for tx_num in 0..tx_per_block {
			...
			let transfer =
				factory_state.transfer_extrinsic(&from.0, &from.1, &to, &amount, version, &genesis_hash, &best_hash);

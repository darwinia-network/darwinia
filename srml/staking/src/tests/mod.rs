...
// --- custom ---
use crate::{
    utils, Exposures, IndividualExpo, RewardDestination, StakingBalance, StakingLedger,
    TimeDepositItem, UnlockChunk, MONTH_IN_SECONDS, STAKING_ID,
};
...
fn time_deposit_ring_unbond_and_withdraw_should_work() {
    with_externalities(
        ...
        || {
            ...
            let _ = Ring::deposit_creating(&11, 1000 * COIN);
            let _ = Staking::unbond(Origin::signed(10), StakingBalance::Ring(10 * COIN));
            ...
            let _ = Staking::unbond(Origin::signed(10), StakingBalance::Ring(20 * COIN));
            ...
            let _ = Staking::unbond(Origin::signed(10), StakingBalance::Ring(120 * COIN));
...
fn normal_unbond_should_work() {
    with_externalities(
        ...
        || {
            ...
            let _ = Staking::bond_extra(Origin::signed(11), StakingBalance::Ring(200 * COIN), 12);

            assert_eq!(
                Kton::free_balance(&11),
                kton_free_balance + 200 * COIN / 10000
            );

            origin_ledger.deposit_items.push(TimeDepositItem {
                value: 200 * COIN,
                start_time: 0,
                expire_time: 12 * MONTH_IN_SECONDS as u64,
            });
            ...
            assert_ok!(Staking::unbond(
                Origin::signed(10),
                StakingBalance::Kton(300 * COIN / 10000)
            ));
        },
    );
}
        ...
        || {
            ...
            assert_eq!(
                ...
                Some(StakingLedger {
                    ...
                    unlocking: vec![UnlockChunk {
                        value: StakingBalance::Ring(5 * COIN),
                        era: 3,
                        is_time_deposit: true,
                    }],
                })
            ...
            assert_eq!(
                Kton::free_balance(&1001),
                kton_free_balance - 3 * kton_punishment
            );
...
fn transform_to_promised_ring_should_work() {
    with_externalities(
        ...
        || {
            ...
            assert_eq!(
                Kton::free_balance(&1001),
                kton_free_balance + (5 * COIN / 10000)
            );
        },
    );
}
            ...
            assert_eq!(
                ...
                Some(StakingLedger {
                    ...
                    deposit_items: vec![TimeDepositItem {
                        value: 5 * COIN,
                        start_time: 33696000,
                        expire_time: 26 * MONTH_IN_SECONDS as u64,
                    }],
                    unlocking: vec![],
                })
            );
...
fn reward_should_work_correctly() {
    with_externalities(
        ...
        || {
            ...
            let _ = Staking::bond_extra(Origin::signed(2001), StakingBalance::Kton(1 * COIN), 0);
            // 1001-1000
            let _ = Staking::bond(
                Origin::signed(1001),
                1000,
            ...
            let _ = Staking::bond_extra(Origin::signed(1001), StakingBalance::Kton(1 * COIN), 0);
            ...
            let _ = Staking::validate(Origin::signed(2000), [0; 8].to_vec(), 0, 3);
            let _ = Staking::nominate(Origin::signed(1000), vec![2001]);
...
fn slash_should_work() {
    with_externalities(
        ...
        || {
            ...
            let _ = Staking::bond_extra(Origin::signed(1000), StakingBalance::Kton(50 * COIN), 0);
            let _ = Staking::validate(Origin::signed(1000), [0; 8].to_vec(), 0, 3);

...
impl<T: Trait> Module<T> {
	...
	fn do_gossip_at(block_number: T::BlockNumber) -> Result<(), OffchainErr> {
		...
		for (authority_index, key) in authorities.into_iter().enumerate().filter_map(|(index, authority)| {
			local_keys
			...
			let network_state = runtime_io::offchain::network_state().map_err(|_| OffchainErr::NetworkState)?;
			...
			results.push(T::SubmitTransaction::submit_unsigned(call).map_err(|_| OffchainErr::SubmitTransaction));
		}
	...
	fn compare_and_set_worker_status(
		...
		let enc = WorkerStatus { done, gossipping_at };
	...
	fn check_not_yet_gossipped(
		...
		match last_gossip {
			Some(last) => {
				...
				let not_yet_gossipped = worker_status.done && worker_status.gossipping_at < next_gossip;

...
pub trait Trait: system::Trait + session::Trait {
    /// The staking-deprecated balance.
...
decl_storage! {
	trait Store for Module<T: Trait> as Staking {
		...
		/// The ideal number of staking-deprecated participants.
		pub ValidatorCount get(validator_count) config(): u32;
		/// Minimum number of staking-deprecated participants before emergency conditions are imposed.
		...
		/// Map from all (unlocked) "controller" accounts to the info regarding the staking-deprecated.
	...
	pub struct Module<T: Trait> for enum Call where origin: T::Origin {
		...
		fn unbond(origin, #[compact] value: BalanceOf<T>) {
			...
			if !value.is_zero() {
				...
				// Avoid there being a dust balance left in the staking-deprecated system.
...
//            <Stakers<T>>::insert(stash, exposure);
    ...
    /// The era has changed - enact new staking-deprecated set.
    ///

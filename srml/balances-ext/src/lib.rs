...
pub type NegativeImbalanceOf<T> = <<T as Trait>::Ring as Currency<<T as system::Trait>::AccountId>>::NegativeImbalance;
pub type PositiveImbalanceOf<T> = <<T as Trait>::Ring as Currency<<T as system::Trait>::AccountId>>::PositiveImbalance;
...
decl_storage! {
    trait Store for Module<T: Trait> as BalancesExt {
...
decl_module! {
    pub struct Module<T: Trait> for enum Call where origin: T::Origin {
        ...
        pub fn deposit(origin, value: RingBalanceOf<T>, months: u32) {
            ...
            let positive_imbalance = T::Kton::deposit_into_existing(&transactor, kton_return);

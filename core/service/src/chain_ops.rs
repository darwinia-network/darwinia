...
macro_rules! import_blocks {
	($block:ty, $client:ident, $queue:ident, $exit:ident, $input:ident) => {{
		use consensus_common::import_queue::{BlockImportError, BlockImportResult, IncomingBlock, Link};
		...
		let count: u64 = Decode::decode(&mut io_reader_input).map_err(|e| format!("Error reading file: {}", e))?;
		...
		Ok(futures::future::poll_fn(move || {
			...
			.poll();
			if link.has_error {
				info!("Stopping after #{} blocks because of an error", link.imported_blocks,);
				return Ok(Async::Ready(()));
				}

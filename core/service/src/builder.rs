...
use crate::{build_network_future, start_rpc_servers, SpawnTaskHandle, TransactionPoolAdapter};
use crate::{
	error::{self, Error},
	NetworkState, NetworkStatus, Service, DEFAULT_PROTOCOL_ID,
};
use chain_spec::{Extension, RuntimeGenesis};
use client::{backend::RemoteBackend, light::blockchain::RemoteBlockchain, BlockchainEvents, Client};
use codec::{Decode, Encode, IoReader};
use consensus_common::import_queue::ImportQueue;
use futures::{prelude::*, sync::mpsc};
use futures03::{compat::Compat, future::ready, FutureExt as _, StreamExt as _, TryFutureExt as _, TryStreamExt as _};
...
use sr_primitives::traits::{
	Block as BlockT, Extrinsic, Header, NumberFor, One, ProvideRuntimeApi, SaturatedConversion, Zero,
};
use std::{
	io::{Read, Seek, Write},
	marker::PhantomData,
...
type TFullCallExecutor<TBl, TExecDisp> = client::LocalCallExecutor<client_db::Backend<TBl>, NativeExecutor<TExecDisp>>;

/// Light client type.
type TLightClient<TBl, TRtApi, TExecDisp> = Client<TLightBackend<TBl>, TLightCallExecutor<TBl, TExecDisp>, TBl, TRtApi>;

/// Light client backend type.
type TLightBackend<TBl> = client::light::backend::Backend<client_db::light::LightStorage<TBl>, Blake2Hasher>;
...
where
	...
	TGen: sr_primitives::BuildStorage + serde::Serialize + for<'de> serde::Deserialize<'de>,
	TCSExt: Extension,
{
	...
	TCSExt: Extension,
{
	...
	let executor = NativeExecutor::<TExecDisp>::new(config.wasm_method, config.default_heap_pages);
	...
	let (client, backend) = {
		let db_config = client_db::DatabaseSettings {
			...
			source: match &config.database {
				DatabaseConfig::Path { path, cache_size } => client_db::DatabaseSettingsSrc::Path {
					path: path.clone(),
					cache_size: cache_size.clone().map(|u| u as usize),
				},
				DatabaseConfig::Custom(db) => client_db::DatabaseSettingsSrc::Custom(db.clone()),
			},
		};
	...
	TCSExt: Extension,
{
	...
	> {
		...
		let client = Arc::new(client::light::new_light(backend.clone(), &config.chain_spec, executor)?);
...
impl<TBl, TRtApi, TCfg, TGen, TCSExt, TCl, TFchr, TSc, TImpQu, TFprb, TFpp, TNetP, TExPool, TRpc, Backend>
	ServiceBuilder<TBl, TRtApi, TCfg, TGen, TCSExt, TCl, TFchr, TSc, TImpQu, TFprb, TFpp, TNetP, TExPool, TRpc, Backend>
{
	...
	TRtApi: 'static + Send + Sync,
{
	fn import_blocks(
		...
		import_blocks!(TBl, client, queue, exit, input).map(|f| Box::new(f) as Box<_>)
	}
}
...
impl<TBl, TRtApi, TCfg, TGen, TCSExt, TBackend, TExec, TSc, TImpQu, TNetP, TExPoolApi, TRpc>
	...
	<Client<TBackend, TExec, TBl, TRtApi> as ProvideRuntimeApi>::Api: sr_api::Metadata<TBl>
	...
	TRpc: rpc::RpcExtension<rpc::Metadata> + Clone,
{
	...
	pub fn build(
		...
		Service<
			...
			TransactionPool<TExPoolApi>,
			offchain::OffchainWorkers<Client<TBackend, TExec, TBl, TRtApi>, TBackend::OffchainStorage, TBl>,
		>,
		Error,
	> {
		let ServiceBuilder {
		...
		let (to_spawn_tx, to_spawn_rx) = mpsc::unbounded::<Box<dyn Future<Item = (), Error = ()> + Send>>();
			...
			let events = client
				...
				.for_each(move |notification| {
					...
					if let (Some(txpool), Some(offchain)) = (txpool, offchain) {
						let future = offchain
							.on_block_imported(&number, &txpool, network_state_info.clone(), is_validator)
							.map(|()| Ok(()));
		...
		let gen_handler = || {
			...
			let (chain, state) =
				if let (Some(remote_backend), Some(on_demand)) = (remote_backend.as_ref(), on_demand.as_ref()) {
					// Light clients
		...
		let telemetry = config.telemetry_endpoints.clone().map(|endpoints| {
			...
			let startup_time = SystemTime::UNIX_EPOCH.elapsed().map(|dur| dur.as_millis()).unwrap_or(0);
			...
			let _ = to_spawn_tx.unbounded_send(Box::new(future.select(exit.clone()).then(|_| Ok(()))));
			telemetry
		});
...
where
	...
	<Client<Backend, Executor, Block, Api> as ProvideRuntimeApi>::Api: tx_pool_api::TaggedTransactionQueue<Block>,
...
mod tests {
	...
	fn should_remove_transactions_from_the_pool() {
		...
		maintain_transaction_pool(&id, &client, &pool, &[])
			.unwrap()
			.wait()
			.unwrap();
	...
	fn should_add_reverted_transactions_to_the_pool() {
		...
		maintain_transaction_pool(&id, &client, &pool, &[block1_hash])
			.unwrap()
			.wait()
			.unwrap();

		...
		.block_on(future::poll_fn(|| -> Result<_, io::Error> {
			loop {
				...
				match service1.poll().unwrap() {
					Async::Ready(Some(LegacyProtoOut::CustomProtocolOpen { .. })) => match service1_state {
					...
					Async::Ready(Some(LegacyProtoOut::CustomProtocolClosed { .. })) => match service1_state {
						ServiceState::FirstConnec => service1_state = ServiceState::Disconnected,
						ServiceState::ConnectedAgain | ServiceState::NotConnected | ServiceState::Disconnected => {
							panic!()
						}
					},
				...
				match service2.poll().unwrap() {
					Async::Ready(Some(LegacyProtoOut::CustomProtocolOpen { .. })) => match service2_state {
						ServiceState::NotConnected => {
					...
					Async::Ready(Some(LegacyProtoOut::CustomProtocolClosed { .. })) => match service2_state {

...
use client::{backend::Backend, error::Result as ClientResult, BlockchainEvents, CallExecutor, Client};
use primitives::{
	...
	fn child_storage_keys(
		...
		Box::new(result(
			self.block_or_best(block)
				.and_then(|block| {
					self.client
						.child_storage_keys(&BlockId::Hash(block), &child_storage_key, &prefix)
				})
				.map_err(client_err),
		))
	}

...
use primitives::{convert_hash, traits::CodeExecutor, ChangesTrieConfiguration, H256};
use sr_primitives::traits::{
	Block as BlockT, CheckedConversion, Hash, HashFor, Header as HeaderT, NumberFor, SimpleArithmetic, Zero,
};
pub use state_machine::StorageProof;
use state_machine::{
	create_proof_check_backend_storage, key_changes_proof_check, read_child_proof_check, read_proof_check,
	ChangesTrieAnchorBlockId, ChangesTrieConfigurationRange, ChangesTrieRootsStorage, TrieBackend,
};
...
impl<E, H, B: BlockT, S: BlockchainStorage<B>> LightDataChecker<E, H, B, S> {
		...
		H::Out: Ord,
	{
		...
		let is_extra_first_root = remote_proof
			...
			.keys()
			.next()
			.map(|first_root| *first_root < request.first_block.0 || *first_root >= request.tries_roots.0)
			.unwrap_or(false);
	...
	S: BlockchainStorage<Block>,
{
	fn check_header_proof(
		...
		let remote_header = remote_header.ok_or_else(|| ClientError::from(ClientError::InvalidCHTProof))?;
		let remote_header_hash = remote_header.hash();
		cht::check_proof::<Block::Header, H>(request.cht_root, request.block, remote_header_hash, remote_proof)
			.map(|_| remote_header)
	}
	...
	fn check_body_proof(
		...
		// TODO: #2621
		let extrinsics_root = HashFor::<Block>::ordered_trie_root(body.iter().map(Encode::encode).collect());
...
where
	H: Hasher,
	Number:
		::std::fmt::Display + ::std::hash::Hash + Clone + SimpleArithmetic + Encode + Decode + Send + Sync + 'static,
	Hash: 'a + Send + Sync + Clone + AsRef<[u8]>,
{
	fn build_anchor(&self, _hash: H::Out) -> Result<state_machine::ChangesTrieAnchorBlockId<H::Out, Number>, String> {
...
pub mod tests {
	...
	use crate::light::fetcher::{FetchChecker, Fetcher, LightDataChecker, RemoteCallRequest, RemoteHeaderRequest};
	...
	type TestChecker = LightDataChecker<NativeExecutor<test_client::LocalExecutor>, Blake2Hasher, Block, DummyStorage>;
	...
	fn prepare_for_read_proof_check() -> (TestChecker, Header, StorageProof, u32) {
		...
		local_storage
			...
			.unwrap();
		let local_checker =
			LightDataChecker::new(Arc::new(DummyBlockchain::new(DummyStorage::new())), local_executor());
	...
	fn prepare_for_read_child_proof_check() -> (TestChecker, Header, StorageProof, Vec<u8>) {
		...
		let remote_read_proof = remote_client
			.read_child_proof(&remote_block_id, b":child_storage:default:child1", &[b"key1"])
			.unwrap();
	...
	fn changes_proof_is_generated_and_checked_when_headers_are_not_pruned() {
		...
		let local_checker = TestChecker::new(Arc::new(DummyBlockchain::new(DummyStorage::new())), local_executor());
	...
	fn changes_proof_is_generated_and_checked_when_headers_are_pruned() {
		...
		let local_cht_root =
			cht::compute_root::<Header, Blake2Hasher, _>(4, 0, remote_roots.iter().cloned().map(|ct| Ok(Some(ct))))
				.unwrap();
		let mut local_storage = DummyStorage::new();
		local_storage.changes_tries_cht_roots.insert(0, local_cht_root);
		let local_checker = TestChecker::new(Arc::new(DummyBlockchain::new(local_storage)), local_executor());

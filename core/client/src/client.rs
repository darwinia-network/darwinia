...
use consensus::{
	self, BlockCheckParams, BlockImportParams, BlockOrigin, BlockStatus, Error as ConsensusError, ForkChoiceStrategy,
...
use primitives::{
	...
	traits::CodeExecutor,
	Blake2Hasher, ChangesTrieConfiguration, ExecutionContext, NativeOrEncoded, NeverNativeValue, H256,
};
use sr_primitives::{
	generic::{BlockId, DigestItem, SignedBlock},
	traits::{
		ApiRef, Block as BlockT, DigestFor, Header as HeaderT, NumberFor, One, ProvideRuntimeApi, SaturatedConversion,
		Zero,
	},
	BuildStorage, Justification,
};
use state_machine::{
	key_changes, key_changes_proof, merge_storage_proofs, prove_child_read, prove_read, Backend as StateBackend,
	BackendTrustLevel, ChangesTrieAnchorBlockId, ChangesTrieConfigurationRange, ChangesTrieRootsStorage,
	ChangesTrieStorage, ChangesTrieTransaction, DBValue, ExecutionManager, ExecutionStrategy, OverlayedChanges,
	StorageProof,
};
use std::{
	cell::RefCell,
...
use sr_api::{CallRuntimeAt, ConstructRuntimeApi, Core as CoreApi, InitializeBlock, ProofRecorder};

use crate::{
	backend::{
		self, BlockImportOperation, ClientImportOperation, Finalizer, ImportSummary, PrunableStateChangesTrieStorage,
	},
	blockchain::{
		self, well_known_cache_keys::Id as CacheKeyId, Backend as ChainBackend, Cache,
		HeaderBackend as ChainHeaderBackend, Info as ChainInfo, ProvideCache,
	},
...
pub fn new_in_mem<E, Block, S, RA>(
	...
	Client<in_mem::Backend<Block, Blake2Hasher>, LocalCallExecutor<in_mem::Backend<Block, Blake2Hasher>, E>, Block, RA>,
>
where
	...
	Block: BlockT<Hash = H256>,
{
	...
	pub fn storage_hash(&self, id: &BlockId<Block>, key: &StorageKey) -> error::Result<Option<Block::Hash>> {
		Ok(self
			.state_at(id)?
			.storage_hash(&key.0)
			.map_err(|e| error::Error::from_state(Box::new(e)))?)
	...
	pub fn read_proof<I>(&self, id: &BlockId<Block>, keys: I) -> error::Result<StorageProof>
	where
		I: IntoIterator,
		I::Item: AsRef<[u8]>,
	{
	...
	/// read proof.
	pub fn read_child_proof<I>(&self, id: &BlockId<Block>, storage_key: &[u8], keys: I) -> error::Result<StorageProof>
	where
		I: IntoIterator,
		I::Item: AsRef<[u8]>,
	{
	...
	pub fn max_key_changes_range(
		...
		if first > last_num {
			return Err(error::Error::ChangesTrieAccessFailed(
	...
	) -> error::Result<block_builder::BlockBuilder<Block, Self>>
	where
		E: Clone + Send + Sync,
		RA: Send + Sync,
		...
		E: CallExecutor<Block, Blake2Hasher> + Send + Sync + Clone,
	{
		...
		let retracted = if is_new_best {
			let route_from_best = header_metadata::tree_route(self.backend.blockchain(), info.best_hash, parent_hash)?;
			route_from_best
				.retracted()
				.iter()
				.rev()
		...
		E: CallExecutor<Block, Blake2Hasher> + Send + Sync + Clone,
	{
		match transaction.state()? {
			Some(transaction_state) => {
				...
				let get_execution_manager = |execution_strategy: ExecutionStrategy| match execution_strategy {
					...
					ExecutionStrategy::Both => ExecutionManager::Both(|wasm_result, native_result| {
						...
						warn!(
							"Consensus error between wasm and native block execution at block {}",
							hash
						);
						warn!("   Header {:?}", header);
				...
				let (_, storage_update, changes_update) =
					self.executor.call_at_state::<_, _, NeverNativeValue, fn() -> _>(
						transaction_state,
						&mut overlay,
						"Core_execute_block",
						&encoded_block,
	...
	fn notify_finalized(&self, notify_finalized: Vec<Block::Hash>) -> error::Result<()> {
	...
	pub fn block(&self, id: &BlockId<Block>) -> error::Result<Option<SignedBlock<Block>>> {
	...
	fn changes_trie_config(&self) -> Result<Option<ChangesTrieConfiguration>, Error> {
		Ok(self
			.backend
			.state_at(BlockId::Number(self.backend.blockchain().info().best_number))?
...
impl<B, E, Block, RA> sr_primitives::traits::BlockIdTo<Block> for Client<B, E, Block, RA>
where
	B: backend::Backend<Block, Blake2Hasher>,
	E: CallExecutor<Block, Blake2Hasher> + Send + Sync,
	Block: BlockT<Hash = H256>,
	...
	Block: BlockT<Hash = H256>,
{
	...
	>(
		...
		let manager = match context {
			...
			ExecutionContext::OffchainCall(_) => self.execution_strategies.other.get_manager(),
		};

		let capabilities = context.capabilities();
		let offchain_extensions = if let ExecutionContext::OffchainCall(Some(ext)) = context {
			Some(OffchainExt::new(offchain::LimitedExternalities::new(
				capabilities,
				ext.0,
			)))
		} else {
...
pub mod utils {
	...
	where
		T: ChainHeaderBackend<Block> + HeaderMetadata<Block, Error = error::Error>,
	{
		move |base, hash| {
			if base == hash {
				return Ok(false);
...
pub(crate) mod tests {
	...
	fn best_containing_on_longest_chain_with_multiple_forks() {
		...
		assert_eq!(None, longest_chain_select.finality_target(a5.hash(), Some(2)).unwrap());
		...
		assert_eq!(None, longest_chain_select.finality_target(c3.hash(), Some(2)).unwrap());
		...
		assert_eq!(None, longest_chain_select.finality_target(a4.hash(), Some(1)).unwrap());
		assert_eq!(None, longest_chain_select.finality_target(a5.hash(), Some(1)).unwrap());

		assert_eq!(None, longest_chain_select.finality_target(b2.hash(), Some(1)).unwrap());
		assert_eq!(None, longest_chain_select.finality_target(b3.hash(), Some(1)).unwrap());
		assert_eq!(None, longest_chain_select.finality_target(b4.hash(), Some(1)).unwrap());

		assert_eq!(None, longest_chain_select.finality_target(c3.hash(), Some(1)).unwrap());

		assert_eq!(None, longest_chain_select.finality_target(d2.hash(), Some(1)).unwrap());
		...
		assert_eq!(None, longest_chain_select.finality_target(a3.hash(), Some(0)).unwrap());
		assert_eq!(None, longest_chain_select.finality_target(a4.hash(), Some(0)).unwrap());
		assert_eq!(None, longest_chain_select.finality_target(a5.hash(), Some(0)).unwrap());

		assert_eq!(None, longest_chain_select.finality_target(b2.hash(), Some(0)).unwrap());
	...
	fn import_with_justification() {
		...
		assert_eq!(client.justification(&BlockId::Hash(a2.hash())).unwrap(), None,);
	}
	...
	fn doesnt_import_blocks_that_revert_finality() {
		...
		let expected_err = ConsensusError::ClientImport(error::Error::NotInFinalizedChain.to_string());

...
impl<Block: BlockT> StorageNotifications<Block> {
	...
	pub fn trigger(
		...
		for subscriber in subscribers {
			let should_remove = {
				let &(ref sink, ref filter, ref child_filters) = self
					.sinks
					.get(&subscriber)
...
mod tests {
	use super::*;
	use sr_primitives::testing::{Block as RawBlock, ExtrinsicWrapper, H256 as Hash};
	...
	fn triggering_change_should_notify_wildcard_listeners() {
		...
		let mut recv = futures03::executor::block_on_stream(notifications.listen(None, Some(&child_filter[..])));
	...
	fn should_only_notify_interested_listeners() {
		...
		let mut recv1 = futures03::executor::block_on_stream(notifications.listen(Some(&[StorageKey(vec![1])]), None));
		let mut recv2 = futures03::executor::block_on_stream(notifications.listen(Some(&[StorageKey(vec![2])]), None));
		let mut recv3 = futures03::executor::block_on_stream(notifications.listen(Some(&[]), Some(&child_filter)));
	...
	fn should_cleanup_subscribers_if_dropped() {
		...
		{
			let child_filter = [(StorageKey(vec![4]), Some(vec![StorageKey(vec![5])]))];
			let _recv1 = futures03::executor::block_on_stream(notifications.listen(Some(&[StorageKey(vec![1])]), None));
			let _recv2 = futures03::executor::block_on_stream(notifications.listen(Some(&[StorageKey(vec![2])]), None));
			let _recv3 = futures03::executor::block_on_stream(notifications.listen(None, None));
			let _recv4 = futures03::executor::block_on_stream(notifications.listen(None, Some(&child_filter)));

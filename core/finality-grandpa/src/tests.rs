...
use codec::Decode;
use consensus_common::import_queue::{BoxBlockImport, BoxFinalityProofImport, BoxJustificationImport};
use consensus_common::{BlockImportParams, BlockOrigin, ForkChoiceStrategy, ImportResult, ImportedAux};
...
use finality_proof::{AuthoritySetForFinalityChecker, AuthoritySetForFinalityProver, FinalityProofProvider};
...
impl TestNetFactory for GrandpaTestNet {
	...
	fn make_verifier(&self, _client: PeersClient, _cfg: &ProtocolConfig, _: &PeerData) -> Self::Verifier {
	...
	fn make_block_import(
		...
		match client {
			PeersClient::Full(ref client, ref backend) => {
				let (import, link) =
					block_import(client.clone(), &self.test_config, LongestChain::new(backend.clone()))
...
impl ApiExt<Block> for RuntimeApi {
	...
	fn map_api_result<F: FnOnce(&Self) -> result::Result<R, E>, R, E>(&self, _: F) -> result::Result<R, E> {
		unimplemented!("Not required for testing!")
	}
...
impl AuthoritySetForFinalityProver<Block> for TestApi {
	...
	fn prove_authorities(&self, block: &BlockId<Block>) -> Result<StorageProof> {
		...
		let backend = <InMemory<Blake2Hasher>>::from(vec![(None, b"authorities".to_vec(), Some(authorities.encode()))]);
		let proof =
			prove_read(backend, vec![b"authorities"]).expect("failure proving read from in-memory storage backend");
		Ok(proof)
	}
}
...
impl AuthoritySetForFinalityChecker<Block> for TestApi {
	fn check_authorities_proof(
		...
		let results = read_proof_check::<Blake2Hasher, _>(*header.state_root(), proof, vec![b"authorities"])
		...
		let authorities = Decode::decode(&mut &encoded[..]).expect("failure decoding authorities read from proof");
		Ok(authorities)
	}
}
	...
	F: FnOnce(current_thread::Handle) -> Option<Box<dyn Future<Item = (), Error = ()>>>,
{
	...
	let wait_for = ::futures::future::join_all(wait_for).map(|_| ()).map_err(|_| ());
...
fn add_forced_change(block: &mut Block, median_last_finalized: BlockNumber, change: ScheduledChange<BlockNumber>) {
	block.header.digest_mut().push(DigestItem::Consensus(
		GRANDPA_ENGINE_ID,
...
fn transition_3_voters_twice_1_full_observer() {
	...
	for (i, peer) in net.lock().peers().iter().enumerate() {
		...
		assert_eq!(full_client.info().chain.best_number, 1, "Peer #{} failed to sync", i);
...
fn consensus_changes_works() {
	...
	assert_eq!(
		changes.finalize((5, H256::from_low_u64_be(5)), |_| Ok(None)).unwrap(),
		(false, false)
	);
	...
	assert_eq!(
		changes
			.finalize((10, H256::from_low_u64_be(10)), |_| Ok(Some(H256::from_low_u64_be(
				1001
			))))
			.unwrap(),
		(true, false)
	...
	assert_eq!(
		changes
			.finalize((10, H256::from_low_u64_be(10)), |_| Ok(Some(H256::from_low_u64_be(1))))
			.unwrap(),
		(true, true)
	);
}
...
fn sync_justifications_on_change_blocks() {
	...
	runtime
		.block_on(futures::future::poll_fn(move || -> std::result::Result<_, ()> {
			if net
				.lock()
				.peer(3)
				.client()
...
fn force_change_to_new_set() {
	...
	for (i, peer) in net.lock().peers().iter().enumerate() {
		assert_eq!(peer.client().info().chain.best_number, 26, "Peer #{} failed to sync", i);
		...
		impl Future for ResettableVoter {
			...
			fn poll(&mut self) -> Poll<Self::Item, Self::Error> {
				...
				match self.voter_rx.poll() {
					...
					Ok(Async::NotReady) => {}
					Ok(Async::Ready(Some(()))) => {
						let (_block_import, _, _, _, link) = self.net.lock().make_block_import(self.client.clone());
					...
					if state.compare_and_swap(0, 1, Ordering::SeqCst) == 0 {
						...
						future::Either::A(
							tokio_timer::Interval::new_interval(Duration::from_millis(200))
								.take_while(move |_| Ok(net2.lock().peer(1).client().info().chain.best_number != 40))
								.for_each(|_| Ok(()))
								.and_then(move |_| {
...
fn voter_catches_up_to_latest_round_when_behind() {
	...
	let voter =
		|keystore, peer_id, link, net: Arc<Mutex<GrandpaTestNet>>| -> Box<dyn Future<Item = (), Error = ()> + Send> {
			let grandpa_params = GrandpaParams {
				config: Config {
					gossip_duration: TEST_GOSSIP_DURATION,
					justification_period: 32,
...
fn grandpa_environment_respects_voting_rules() {
	...
	let environment = |voting_rule: Box<dyn VotingRule<Block, TestClient>>| {
		...
		let (network, _) = NetworkBridge::new(network_service.clone(), config.clone(), set_state.clone(), Exit);
...
fn imports_justification_for_regular_blocks_on_import() {
	...
	assert!(client.justification(&BlockId::Hash(block_hash)).unwrap().is_some(),);
}

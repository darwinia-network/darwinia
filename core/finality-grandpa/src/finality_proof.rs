	...
	B: Backend<Block, Blake2Hasher> + Send + Sync + 'static,
{
	...
	pub fn new(backend: Arc<B>, authority_provider: Arc<dyn AuthoritySetForFinalityProver<Block>>) -> Self {
...
where
	...
	B: Backend<Block, Blake2Hasher> + Send + Sync + 'static,
{
	fn prove_finality(&self, for_block: Block::Hash, request: &[u8]) -> Result<Option<Vec<u8>>, ClientError> {
		let request: FinalityProofRequest<Block::Hash> = Decode::decode(&mut &request[..]).map_err(|e| {
			...
			if justifies_end_block || justifies_authority_set_change {
				...
				if finality_proof.is_empty() {
					let justification_check_result =
						J::decode_and_verify(&proof_fragment.justification, authorities_set_id, &current_authorities);
	...
	J: ProvableJustification<Block::Header>,
{
	...
	for (proof_fragment_index, proof_fragment) in proof.into_iter().enumerate() {
		...
		authorities =
			check_finality_proof_fragment::<_, _, J>(blockchain, authorities, authorities_provider, proof_fragment)?;
	}
	...
	J: Decode + ProvableJustification<Block::Header>,
{
	...
	let justification: J =
		Decode::decode(&mut &proof_fragment.justification[..]).map_err(|_| ClientError::JustificationDecode)?;
	...
	if let Some(new_authorities_proof) = proof_fragment.authorities_proof {
		...
		current_authorities =
			authorities_provider.check_authorities_proof(parent_hash, parent_header, new_authorities_proof)?;
...
pub(crate) trait ProvableJustification<Header: HeaderT>: Encode + Decode {
	...
	fn decode_and_verify(
		...
		let justification = Self::decode(&mut &**justification).map_err(|_| ClientError::JustificationDecode)?;
			...
			&mut &prove_finality::<_, _, TestJustification>(
				...
				&(
					|block_id| match block_id {
						BlockId::Hash(h) if h == header(3).hash() => {
							Ok(vec![(AuthorityId::from_slice(&[3u8; 32]), 1u64)])
						}

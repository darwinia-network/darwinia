...
use client::{
	...
	BlockchainEvents, CallExecutor, Client, ProvideUncles,
};
use consensus_common::import_queue::{BasicQueue, CacheKeyId, Verifier};
use consensus_common::import_queue::{BoxFinalityProofImport, BoxJustificationImport};
use consensus_common::ImportResult;
use consensus_common::SelectChain;
pub use consensus_common::SyncOracle;
use consensus_common::{
	self, BlockCheckParams, BlockImport, BlockImportParams, BlockOrigin, Environment, Error as ConsensusError,
	ForkChoiceStrategy, Proposer,
};
use inherents::{InherentData, InherentDataProviders};
use keystore::KeyStorePtr;
use parking_lot::Mutex;
use primitives::{Blake2Hasher, Pair, H256};
use sr_primitives::traits::{Block as BlockT, DigestItemFor, Header, ProvideRuntimeApi, Zero};
use sr_primitives::{
	generic::{BlockId, OpaqueDigestItemId},
	Justification,
};
use srml_babe::{
	timestamp::{InherentType as TimestampInherent, TimestampInherentData},
	BabeInherentData,
};
use std::{
	collections::HashMap,
...
use substrate_telemetry::{telemetry, CONSENSUS_DEBUG, CONSENSUS_TRACE};
...
use slots::{check_equivocation, CheckedHeader};
use slots::{SlotCompatible, SlotData, SlotInfo, SlotWorker};
...
mod tests;
mod verification;
pub use babe_primitives::{AuthorityId, AuthorityPair, AuthoritySignature, Epoch, NextEpochDescriptor};
...
impl Config {
	...
	where
		...
		C::Api: BabeApi<B, Error = client::error::Error>,
	{
...
pub fn start_babe<B, C, SC, E, I, SO, Error>(
	BabeParams {
		keystore,
		client,
		select_chain,
	...
	SO: SyncOracle + Send + Sync + Clone,
{
	...
	uncles::register_uncles_inherent_data_provider(client.clone(), select_chain.clone(), &inherent_data_providers)?;
...
fn find_next_epoch_digest<B: BlockT>(header: &B::Header) -> Result<Option<NextEpochDescriptor>, Error<B>>
where
...
where
	...
	PRA::Api: BlockBuilderApi<Block, Error = client::error::Error> + BabeApi<Block, Error = client::error::Error>,
{
	fn verify(
		&mut self,
		origin: BlockOrigin,
		...
		match verification::check_header::<Block>(v_params)? {
			CheckedHeader::Checked(pre_header, verified_info) => {
				...
				if let Some(equivocation_proof) =
					check_equivocation(&*self.api, slot_now, babe_pre_digest.slot_number(), &header, &author)
	...
	PRA::Api: BabeApi<Block>,
{
	...
	fn import_block(
		...
		let parent_header = self
			...
			.ok_or_else(|| {
				ConsensusError::ChainLookup(babe_err(Error::<Block>::ParentUnavailable(parent_hash, hash)).into())
			})?;
		...
		if slot_number <= parent_slot {
			return Err(ConsensusError::ClientImport(
				babe_err(Error::<Block>::SlotNumberMustIncrease(parent_slot, slot_number)).into(),
			));
		}
		...
		let (epoch, first_in_epoch, parent_weight) = {
			...
			let epoch = epoch_changes
				...
				.map_err(|e: fork_tree::Error<client::error::Error>| {
					ConsensusError::ChainLookup(babe_err(Error::<Block>::CouldNotLookUpEpoch(Box::new(e))).into())
				})?
				.ok_or_else(|| ConsensusError::ClientImport(babe_err(Error::<Block>::BlockNotValid(hash)).into()))?;
		...
		let next_epoch_digest =
			find_next_epoch_digest::<Block>(&block.header).map_err(|e| ConsensusError::ClientImport(e.to_string()))?;
		...
		block.fork_choice = {
			...
			let last_best_weight = if &last_best == block.header.parent_hash() {
				...
				aux_schema::load_block_weight(&*self.client, last_best)
					.map_err(|e| ConsensusError::ChainLookup(format!("{:?}", e)))?
					.ok_or_else(|| ConsensusError::ChainLookup(format!("No block weight for parent header.")))?
			};

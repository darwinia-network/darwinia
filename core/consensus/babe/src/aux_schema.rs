...
where
	...
	T: Decode,
{
	let corrupt = |e: codec::Error| ClientError::Backend(format!("BABE DB is corrupted. Decode error: {}", e.what()));
...
pub(crate) fn load_epoch_changes<Block: BlockT, B: AuxStore>(backend: &B) -> ClientResult<SharedEpochChanges<Block>> {
...
pub(crate) fn write_epoch_changes<Block: BlockT, F, R>(epoch_changes: &EpochChangesFor<Block>, write_aux: F) -> R
where
...
pub(crate) fn write_block_weight<H: Encode, F, R>(block_hash: H, block_weight: &BabeBlockWeight, write_aux: F) -> R
where

...
use libp2p::wasm_ext;
use log::{debug, info};
use substrate_service::{config::DatabaseConfig, AbstractService, Configuration, Roles as ServiceRoles, RpcSession};
...
fn start_inner(wasm_ext: wasm_ext::ffi::Transport) -> Result<Client, Box<dyn std::error::Error>> {
	...
	let config = {
		...
		let chain_spec = ChainSpec::CrayfishTestnet.load().map_err(|e| format!("{:?}", e))?;

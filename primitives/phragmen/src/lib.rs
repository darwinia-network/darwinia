...
use sp_runtime::traits::{Bounded, Convert, Member, Saturating, SimpleArithmetic, Zero};
		...
		for n in &voters {
			for e in &n.edges {
				...
				if !c.elected && !c.approval_stake.is_zero() {
					let temp_n =
						multiply_by_rational(n.load.n(), n.budget, c.approval_stake).unwrap_or(Bounded::max_value());
					let temp_d = n.load.d();
				...
				let per_bill_parts = {
					if n.load == e.load {
						...
						if e.load.d() == n.load.d() {
							...
							multiply_by_rational(desired_scale, e.load.n(), n.load.n()).unwrap_or(Bounded::max_value())
						} else {

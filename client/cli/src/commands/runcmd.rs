...
use regex::Regex;
use sc_service::{
	config::KeystoreConfig, AbstractService, ChainSpec, ChainSpecExtension, Configuration, Roles, RuntimeGenesis,
};
...
impl RunCmd {
	...
	fn load_from_file(&mut self) -> error::Result<()> {
		if self.conf.is_none() {
			return Ok(());
		}

		*self = {
			let f = fs::File::open(self.conf.as_ref().unwrap())?;
		...
		F: FnOnce(&str) -> Result<Option<ChainSpec<G, E>>, String>,
	{
		...
		let path = self
			.keystore_path
			.clone()
			.or(config.in_chain_config_dir(DEFAULT_KEYSTORE_CONFIG_PATH));
		...
		let is_authority = (self.validator || self.sentry || is_dev || keyring.is_some()) && !is_light;
		let role = if is_light {
		...
		self.network_config
			.update_config(&mut config, network_path, client_id, is_dev)?;

		self.pool_config.update_config(&mut config)?;

		config.dev_key_seed = keyring.map(|a| format!("//{}", a)).or_else(|| {
			if is_dev && !is_light {
				Some("//Alice".into())
			} else {
				None
		...
		if self.no_telemetry {
			...
			config.telemetry_endpoints = Some(TelemetryEndpoints::new(self.telemetry_endpoints.clone()));
		}
		...
		if self.no_prometheus {
			...
			config.prometheus_port = Some(parse_address(
...
impl Default for RunCmd {
	fn default() -> Self {
		Self {
			...
			offchain_worker: OffchainWorkerEnabled::WhenValidating,
...
mod tests {
	...
	fn keystore_path_is_generated_correctly() {
		let chain_spec = ChainSpec::from_genesis("test", "test-id", || (), Vec::new(), None, None, None, None::<()>);
		...
		for keystore_path in vec![None, Some("/keystore/path")] {
			...
			cli.update_config(&mut config, move |_| Ok(Some(chain_spec)), TEST_VERSION_INFO)
				.unwrap();
	...
	fn ensure_update_config_for_running_node_provides_defaults() {
		let chain_spec = ChainSpec::from_genesis("test", "test-id", || (), vec![], None, None, None, None::<()>);

...
fn force_unstake_works() {
	...
	ExtBuilder::default().build().execute_with(|| {
		...
		assert_noop!(Staking::force_unstake(Origin::signed(11), 11), DispatchError::BadOrigin);
...
fn basic_setup_works() {
	...
	ExtBuilder::default().build().execute_with(|| {
		...
		if cfg!(feature = "equalize") {
			...
			// assert_eq!(Staking::stakers(11).total_ring_balance, 1250);
			// assert_eq!(Staking::stakers(11).others[0].who, 101);
			// assert_eq!(Staking::stakers(11).others[0].ring_balance, 250);
			// assert_eq!(Staking::stakers(21).own_ring_balance, 1000);
			// assert_eq!(Staking::stakers(21).total_ring_balance, 1250);
			// assert_eq!(Staking::stakers(21).others[0].who, 101);
			// assert_eq!(Staking::stakers(21).others[0].ring_balance, 250);
			// assert_eq!(Staking::slot_stake(), 1250);
		} else {
			assert_eq!(Staking::stakers(11).own_ring_balance, 1000);
			assert_eq!(Staking::stakers(11).total_ring_balance, 1125);
			assert_eq!(Staking::stakers(11).others[0].who, 101);
			assert_eq!(Staking::stakers(11).others[0].ring_balance, 125);
			assert_eq!(Staking::stakers(21).own_ring_balance, 1000);
			assert_eq!(Staking::stakers(21).total_ring_balance, 1375);
			assert_eq!(Staking::stakers(21).others[0].who, 101);
			assert_eq!(Staking::stakers(21).others[0].ring_balance, 375);
	...
	ExtBuilder::default().nominate(false).build().execute_with(|| {
		...
		<Stakers<Test>>::insert(
			...
			Exposure {
				...
				total_ring_balance: 1000,
				own_kton_balance: 0,
				own_power: 0,
				total_power: 0,
				total_kton_balance: 0,
		...
		System::set_block_number(block * 5000); // on time.
...
fn staking_should_work() {
	...
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			// TODO:
			// may cause by `fn compute_total_payout` of `inflation.rs`
			// --- Block 2:
			start_session(2);
	...
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			if cfg!(feature = "equalize") {
				...
				// total expo of 10, with 1200 coming from nominators (externals), according to phragmen.
				assert_eq!(Staking::stakers(11).own_ring_balance, 1000);
				assert_eq!(Staking::stakers(11).total_ring_balance, 1000 + 800);
				// 2 and 4 supported 10, each with stake 600, according to phragmen.
				assert_eq!(
					Staking::stakers(11)
						.others
						.iter()
						.map(|e| e.ring_balance)
						.collect::<Vec<mock::Balance>>(),
					vec![400, 400]
				);
				assert_eq!(
					Staking::stakers(11).others.iter().map(|e| e.who).collect::<Vec<u64>>(),
				...
				// total expo of 20, with 500 coming from nominators (externals), according to phragmen.
				assert_eq!(Staking::stakers(21).own_ring_balance, 1000);
				assert_eq_error_rate!(Staking::stakers(21).total_ring_balance, 1000 + 1200, 2);
				// 2 and 4 supported 20, each with stake 250, according to phragmen.
				assert_eq!(
					Staking::stakers(21)
						.others
			...
			assert_eq!(Staking::stakers(31).total_ring_balance, 0);
			assert_eq!(Staking::stakers(41).total_ring_balance, 0);
...
fn nominators_also_get_slashed() {
	...
	ExtBuilder::default().nominate(false).build().execute_with(|| {
		...
		let slash_value = 50;
		let total_slash = expo.total_ring_balance.min(slash_value);
...
fn cannot_transfer_staked_balance() {
	...
	ExtBuilder::default().nominate(false).build().execute_with(|| {
		...
		assert_eq!(Staking::stakers(&11).total_ring_balance, 1000);
...
fn cannot_transfer_staked_balance_2() {
	...
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_eq!(Staking::stakers(&21).total_ring_balance, 1000);
...
fn validator_payment_prefs_work() {
	...
	ExtBuilder::default().build().execute_with(|| {
		...
		// // TODO: fix current_total_payout_for_duration
		// // Compute total payout now for whole duration as other parameter won't change
		// let total_payout_0 = current_total_payout_for_duration(3000);
		// assert!(total_payout_0 > 100); // Test is meaningfull if reward something
		// <Module<Test>>::reward_by_ids(vec![(11, 1)]);
...
fn bond_extra_works() {
	...
	ExtBuilder::default().build().execute_with(|| {
		...
		assert_ok!(Staking::bond_extra(
			Origin::signed(11),
			StakingBalance::RingBalance(u64::max_value()),
			0
		));
...
// Note: rm bond_extra_and_withdraw_unbonded_works, because we are not using this function
...
fn too_many_unbond_calls_should_not_work() {
	ExtBuilder::default().build().execute_with(|| {
		...
		assert_ok!(Staking::unbond(Origin::signed(10), StakingBalance::RingBalance(1)));
		// // TODO: checkout this should fail or not
		// can't do more.
		// assert_noop!(
		...
		// 	Error::<Test>::NoMoreChunks
		// );
		// // TODO: checkout the free up issues
...
fn slot_stake_is_least_staked_validator_and_exposure_defines_maximum_punishment() {
	...
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_eq!(Staking::stakers(&11).total_ring_balance, 1000);
			assert_eq!(Staking::stakers(&21).total_ring_balance, 2000);
			...
			// TODO: checkout why this not work
			// assert_eq!(
			// 	Staking::slot_stake() as u64,
			// 	Staking::stakers(&11).total_ring_balance as u64
			// );
			...
			// 	},
			// );
			// assert_eq!(Staking::stakers(&21).total_ring_balance, 69);
			// <Ledger<Test>>::insert(
			// 	&20,
			// 	StakingLedger {
			// 		stash: 22,
			...
			// // -- new balances + reward
			// assert_eq!(Staking::stakers(&11).total_ring_balance, 1000 + total_payout_0 / 2);
			// assert_eq!(Staking::stakers(&21).total_ring_balance, 69 + total_payout_0 / 2);
...
fn on_free_balance_zero_stash_removes_validator() {
	...
	ExtBuilder::default().existential_deposit(10).build().execute_with(|| {
		...
		let _ = Ring::slash(&10, u64::max_value());
		...
		let _ = Ring::slash(&11, u64::max_value());
...
fn bond_with_no_staked_value() {
	...
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_ok!(Staking::unbond(Origin::signed(2), StakingBalance::RingBalance(5)));
...
fn phragmen_linear_worse_case_equalize() {
	ExtBuilder::default()
		...
		.build()
		.execute_with(|| {
			bond_validator(50, StakingBalance::RingBalance(1000));
			bond_validator(60, StakingBalance::RingBalance(1000));
			bond_validator(70, StakingBalance::RingBalance(1000));

			bond_nominator(2, StakingBalance::RingBalance(2000), vec![11]);
			bond_nominator(4, StakingBalance::RingBalance(1000), vec![11, 21]);
			bond_nominator(6, StakingBalance::RingBalance(1000), vec![21, 31]);
			bond_nominator(8, StakingBalance::RingBalance(1000), vec![31, 41]);
			bond_nominator(110, StakingBalance::RingBalance(1000), vec![41, 51]);
			bond_nominator(120, StakingBalance::RingBalance(1000), vec![51, 61]);
			bond_nominator(130, StakingBalance::RingBalance(1000), vec![61, 71]);
			...
			// TODO: checkout on this
			// assert_eq_error_rate!(Staking::stakers(11).total_ring_balance, 3000, 2);
			// assert_eq_error_rate!(Staking::stakers(21).total_ring_balance, 2255, 2);
			// assert_eq_error_rate!(Staking::stakers(31).total_ring_balance, 2255, 2);
			// assert_eq_error_rate!(Staking::stakers(41).total_ring_balance, 1925, 2);
			// assert_eq_error_rate!(Staking::stakers(51).total_ring_balance, 1870, 2);
			// assert_eq_error_rate!(Staking::stakers(61).total_ring_balance, 1890, 2);
			// assert_eq_error_rate!(Staking::stakers(71).total_ring_balance, 1800, 2);
...
// 		// check_exposure_all();
// 		assert_eq!(Staking::stakers(3).total_ring_balance, u64::max_value());
// 		assert_eq!(Staking::stakers(5).total_ring_balance, u64::max_value());
// 	})
// }
...
// 		bond_validator(2, StakingBalance::RingBalance(u64::max_value() / 2));
// 		bond_validator(4, StakingBalance::RingBalance(u64::max_value() / 2));

// 		bond_nominator(6, StakingBalance::RingBalance(u64::max_value()), vec![3, 5]);
// 		bond_nominator(8, StakingBalance::RingBalance(u64::max_value()), vec![3, 5]);
...
// TODO: check why overflow here
// thread 'tests::phragmen_should_not_overflow_ultimate' panicked at 'attempt to add with overflow',
// #[test]
...
// TODO: check why overflow here
//thread 'tests::reward_validator_slashing_validator_doesnt_overflow' panicked at 'assertion failed: `(left == right)`
//left: `8589934590`,
//right: `17179869180`', frame/staking/src/tests.rs:1936:9
// #[test]
...
// 		assert_eq!(Ring::total_balance(&11), stake - 1);
...
// TODO: mock slashable_balance_of
// #[test]
...
// 16: darwinia_staking::StakingLedger<AccountId,RingBalance,KtonBalance,BlockNumber,Timestamp>::slash::{{closure}}
// at frame/staking/src/lib.rs:493
// 17: darwinia_staking::StakingLedger<AccountId,RingBalance,KtonBalance,BlockNumber,Timestamp>::slash
// at frame/staking/src/lib.rs:534
// 18: darwinia_staking::slashing::do_slash
// at frame/staking/src/slashing.rs:684
// 19: darwinia_staking::slashing::apply_slash
// #[test]
...
//            at frame/staking/src/slashing.rs:684
// 19: darwinia_staking::slashing::apply_slash
//            at frame/staking/src/slashing.rs:732
// #[test]
...
// 		// TODO: the slash may not correct
...
// TODO: check on this
// thread 'tests::slash_in_old_span_does_not_deselect' panicked at 'cannot slash in era 1', frame/staking/src/mock.rs:551:9
// #[test]
...
// 		// TODO: fail here
// 		assert_eq!(Staking::stakers(&11).total_ring_balance, initial_balance);
...
// TODO: checkout slash over flow issues
// #[test]
...
fn remove_deferred() {
	ExtBuilder::default().slash_defer_duration(2).build().execute_with(|| {
		...
		// TODO: checkout why slash in era 1
		// on_offence_in_era(
		// 	&[OffenceDetails {
		// 		offender: (11, exposure.clone()),
		// 		reporters: vec![],
...
// 			StakingRing::RingBalance(unbond_value),
// 		));
// 		assert_eq!(
// 			Ring::locks(stash),
// 			vec![BalanceLock {
...
fn transform_to_deposited_ring_should_work() {
	ExtBuilder::default().build().execute_with(|| {
		...
		// TODO: following error occur
		// left: `Err(DispatchError::Module { index: 0, error: 1, message: Some("NotStash") })`,
...
// TODO: fail need to check on this
// #[test]
...
fn two_different_bond_then_unbond_specific_one() {
	ExtBuilder::default().build().execute_with(|| {
		...
		// TODO: kton free balance not correct

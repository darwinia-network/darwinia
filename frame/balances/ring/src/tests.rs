	...
	($test:ty, $ext_builder:ty, $existential_deposit:expr) => {
		...
		fn basic_locking_should_work() {
			<$ext_builder>::default()
				...
				.execute_with(|| {
					...
					Balances::set_lock(ID_1, &1, LockFor::Common { amount: 9 }, WithdrawReasons::all());
					assert_noop!(
		...
		fn partial_locking_should_work() {
			<$ext_builder>::default()
				...
				.build()
				.execute_with(|| {
					Balances::set_lock(ID_1, &1, LockFor::Common { amount: 5 }, WithdrawReasons::all());
		...
		fn double_locking_should_work() {
			<$ext_builder>::default()
				...
				.execute_with(|| {
					Balances::set_lock(ID_1, &1, LockFor::Common { amount: 5 }, WithdrawReasons::all());
					Balances::set_lock(ID_2, &1, LockFor::Common { amount: 5 }, WithdrawReasons::all());
					assert_ok!(<Balances as Currency<_>>::transfer(&1, &2, 1, AllowDeath));
				});
		}
		...
		fn combination_locking_should_work() {
			<$ext_builder>::default()
				...
				.execute_with(|| {
					...
					Balances::set_lock(ID_2, &1, LockFor::Common { amount: 0 }, WithdrawReasons::all());
					assert_ok!(<Balances as Currency<_>>::transfer(&1, &2, 1, AllowDeath));
				});
		}
		...
		// 		.build()
		// 		.execute_with(|| {
		// 			Balances::set_lock(ID_1, &1, LockFor::Common { amount: 10 }, WithdrawReasons::all());
		// 			assert_noop!(
		// 				<Balances as Currency<_>>::transfer(&1, &2, 6, AllowDeath),
		// 				Error::<$test, _>::LiquidityRestrictions
		// 			);
		...
		fn refunding_balance_should_work() {
			<$ext_builder>::default().build().execute_with(|| {
				let _ = Balances::deposit_creating(&1, 42);
				Balances::mutate_account(&1, |a| a.reserved_ring = 69);

...
use sp_runtime::{
	traits::{AtLeast32Bit, Convert, MaybeSerializeDeserialize, StaticLookup, Zero},
...
impl<Balance: AtLeast32Bit + Copy, BlockNumber: AtLeast32Bit + Copy> VestingInfo<Balance, BlockNumber> {
...
impl<T: Trait> Module<T> {
	...
	fn update_lock(who: T::AccountId) -> DispatchResult {
		...
		if locked_now.is_zero() {
			...
			let reasons = WithdrawReason::Transfer | WithdrawReason::Reserve;
			T::Currency::set_lock(VESTING_ID, &who, LockFor::Common { amount: locked_now }, reasons);
...
mod tests {
	...
	use sp_runtime::{
		testing::Header,
		traits::{BlakeTwo256, Identity, IdentityLookup, OnInitialize},
		Perbill,
	};
	...
	impl frame_system::Trait for Test {
		...
		type Version = ();
		type ModuleToIndex = ();
		type AccountData = pallet_balances::balance::AccountData<u64>;
		type OnNewAccount = ();
